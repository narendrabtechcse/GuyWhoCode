package CourseraDataStructure.week1.week6;

import java.util.*;
import java.io.*;

public class tree_orders {

	static class FastScanner {
		StringTokenizer tok = new StringTokenizer("");
		BufferedReader in;

		FastScanner() {
			in = new BufferedReader(new InputStreamReader(System.in));
		}

		String next() throws IOException {
			while (!tok.hasMoreElements())
				tok = new StringTokenizer(in.readLine());
			return tok.nextToken();
		}

		int nextInt() throws IOException {
			return Integer.parseInt(next());
		}
	}

	public static class TreeOrders {
		int n;
		int[] key, left, right;

		void read() throws IOException {
			FastScanner in = new FastScanner();
			n = in.nextInt();
			key = new int[n];
			left = new int[n];
			right = new int[n];
			for (int i = 0; i < n; i++) {
				key[i] = in.nextInt();
				left[i] = in.nextInt();
				right[i] = in.nextInt();
			}
		}

		ArrayList<Integer> inOrder() {
			ArrayList<Integer> result = new ArrayList<Integer>();
			inOrder(0,result);

			return result;
		}

		void inOrder(int i,ArrayList<Integer> result){
			if(i==-1){
				return;
			}
			inOrder(left[i],result);
			result.add(key[i]);
			inOrder(right[i],result);
		}


		ArrayList<Integer> preOrder() {
			ArrayList<Integer> result = new ArrayList<Integer>();
			preOrder(0,result);

			return result;
		}

		void preOrder(int i,ArrayList<Integer> result){
			if(i==-1){
				return;
			}
			result.add(key[i]);
			preOrder(left[i],result);
			preOrder(right[i],result);
		}

		ArrayList<Integer> postOrder() {
			ArrayList<Integer> result = new ArrayList<Integer>();
			postOrder(0,result);

			return result;
		}

		void postOrder(int i,ArrayList<Integer> result){
			if(i==-1){
				return;
			}
			postOrder(left[i],result);
			postOrder(right[i],result);
			result.add(key[i]);
		}
	}

	static public void main(String[] args) throws IOException {
		new Thread(null, new Runnable() {
			public void run() {
				try {
					new tree_orders().run();
				} catch (IOException e) {
				}
			}
		}, "1", 1 << 26).start();
	}

	public void print(List<Integer> x) {
		for (Integer a : x) {
			System.out.print(a + " ");
		}
		System.out.println();
	}

	public void run() throws IOException {
		TreeOrders tree = new TreeOrders();
		tree.read();
		print(tree.inOrder());
		print(tree.preOrder());
		print(tree.postOrder());
	}
}

